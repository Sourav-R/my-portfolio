...
import { Shield, Activity, Network, ChevronRight, Lock, Database, GitBranch, X, CheckCircle2 } from 'lucide-react';
...
const missions = [
  {
    ...
    description: 'Engineered a multi-node Elastic Security cluster from bare metal and integrated a "source of truth" GLPI ITAM platform to contextualize enterprise SIEM alerts.',
...
[STATUS] 390+ tables initialized via CLI. SLA mapping synced.`,
    extendedDetails: {
      situation: 'Required a centralized "source of truth" to contextualize real-time Elastic Security alerts and track asset data across a distributed enterprise network.',
      actions: [
  ...
  },
  {
    ...
    description: 'Engineered an automated pipeline bridging Elastic Security and Logstash to detect Living-off-the-Land (LotL) threats and forward alerts via SMTP.',
...
[STATUS] Structured SMTP alert forwarded to SOC.`,
  ...
  },
  {
    ...
    color: 'blue',
    cols: 'col-span-1 md:col-span-1',
    description: 'Deployed a high-interaction deception platform with 30+ Docker honeypots to safely capture and log modern automated attack scripts.',
...
[VISUALIZE] Enriched telemetry streaming to Elasticsearch.`,
  ...
  },
  {
    ...
    description: 'Engineering real-time Elastic Dashboards to visualize daily index capacity while architecting automated GLPI network discovery sweeps.',
...
[STATUS] CMH agent deployments actively polling endpoints.`,
    extendedDetails: {
      situation: 'Required continuous monitoring of massive telemetry ingest rates and automated tracking of unauthorized/untracked assets across physical network segments.',
      actions: [
...
const ActiveMissions = () => {
  ...
  return (
    <section id="active-missions" className="relative px-4 py-8 md:py-12 overflow-hidden" ref={containerRef}>
          ...
          {missions.map((mission, index) => {
            ...
            return (
              <motion.div
                ...
                animate={isInView ? { opacity: 1, y: 0, rotate: 0 } : { opacity: 0, y: 40, rotate: 2 }}
              ...
              >
                <TiltCard intensity={8} scale={1.02} className="h-full cursor-pointer">
                  <div className="glass-card h-full relative overflow-hidden p-6 flex flex-col group hover:border-gray-500 transition-colors">
                    {/* Background glow */}
                    <div className={`absolute top-0 right-0 w-64 h-64 bg-${mission.color}-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:bg-${mission.color}-500/20 transition-colors duration-500`} />
                    
                    <div className="relative z-10 flex flex-col h-full pointer-events-none">
                      <div className="flex justify-between items-start mb-6">
                        <div className={`w-12 h-12 bg-black/40 border border-white/10 rounded-lg flex items-center justify-center group-hover:border-${mission.color}-500/30 transition-colors`}>
                          <Icon className={`w-6 h-6 text-${mission.color}-500`} />
                        </div>
                        <span className="text-[10px] font-mono text-gray-600 tracking-widest bg-white/5 px-2 py-1 rounded">PROD_READY</span>
                      </div>

                      <h3 className="text-xl md:text-2xl font-bold text-gray-100 mb-1 leading-tight group-hover:text-white transition-colors">{mission.title}</h3>
                      <p className={`text-xs font-mono text-${mission.color}-400 mb-4 tracking-wide`}>&gt; {mission.subtitle}</p>
                      ...
                      <div className="flex flex-wrap gap-2 mt-auto">
                        {mission.stats.map((stat, i) => {
                          const iconMeta = statIcons[stat] || { type: 'lucide', icon: Activity, color: '#a0a0a0' };
                          return (
                            <span key={i} className="flex items-center gap-1.5 text-[10px] font-mono font-bold text-gray-300 bg-white/5 group-hover:bg-white/10 transition-colors px-2.5 py-1.5 border border-white/5 rounded-sm shadow-sm cursor-default pointer-events-auto">
                              {iconMeta.type === 'brand' ? (
                                <img src={`https://cdn.simpleicons.org/${iconMeta.slug}/${iconMeta.color.replace('#', '')}`} alt={stat} className="w-3.5 h-3.5 opacity-90" style={{ filter: `drop-shadow(0 0 4px #${iconMeta.color.replace('#', '')}60)` }} />
                              ) : (
                                <iconMeta.icon size={14} color={iconMeta.color} className="opacity-90" style={{ filter: `drop-shadow(0 0 4px ${iconMeta.color}60)` }} />
                              )}
                              {stat}
                            </span>
                          );
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 bg-[#080808]">
                      <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-lg font-bold text-white font-mono">{selectedMission.title}</h2>
                      </div>
                      <button
                        onClick={() => setSelectedMission(null)}
                    ...
                    <div className="overflow-y-auto p-6 md:p-8 custom-scrollbar">
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                          ...
                          <div className="bg-[#0a0a0a] rounded-lg p-5 border border-gray-800/50 relative overflow-hidden">
                            <div className={`absolute left-0 top-0 bottom-0 w-1 bg-${mColor}-500`} />
                            <h4 className="text-[10px] uppercase font-mono tracking-widest text-gray-500 mb-2 flex items-center gap-2">
                              <span className={`w-2 h-2 rounded-full bg-${mColor}-500 animate-pulse`} />
                          ...
                          <div className="space-y-4">
                            <h4 className="text-[10px] uppercase font-mono tracking-widest text-gray-500 flex items-center gap-2">
                              <CheckCircle2 className={`w-3 h-3 text-${mColor}-500`} />
                        ...
                        <div className="space-y-6">
                          ...
                          {/* Impact */}
                          <div className={`bg-[#080808] rounded-lg p-5 border border-${mColor}-500/30 group`}>
                            <h4 className={`text-[10px] uppercase font-mono tracking-widest text-${mColor}-400 mb-3`}>
                              [R] Result / Impact
                            </h4>
                          ...
                          {/* Tech Stack List */}
                          <div>
                            <h4 className="text-[10px] uppercase font-mono tracking-widest text-gray-500 mb-3">Core Engine</h4>
                            <div className="flex flex-wrap gap-2">
                              {selectedMission.stats.map((tech, i) => (
                                <span key={i} className={`text-xs px-2.5 py-1 bg-[#111] border border-${mColor}-500/20 text-${mColor}-100 rounded-md font-mono`}>
                                  {tech}
                                </span>
                              ))}
                            </div>
                      ...
                      <div className="mt-8 border border-gray-800 rounded-lg overflow-hidden bg-[#030303]">
                        <div className="flex items-center px-4 py-2 border-b border-gray-800 bg-[#0a0a0a]">
                          ...
                          <span className="text-xs font-mono text-gray-500">{selectedMission.id}_logstream.sh</span>
                        </div>
                        <div className="p-5 font-mono text-sm leading-relaxed space-y-4">
                          {selectedMission.log.split('\n').map((line, idx) => {
                             let lineStyle = "text-gray-400";
                             if (line.includes('[ALERT]') || line.includes('[ATTACK]')) lineStyle = "text-red-400";
                             else if (line.includes('[SYS_LOG]') || line.includes('[ACTION]') || line.includes('[AUTOMATION]')) lineStyle = "text-emerald-400";
                             else if (line.includes('[PIPELINE]') || line.includes('[EQL]') || line.includes('[CAPTURE]')) lineStyle = "text-cyan-400";
                             else if (line.includes('[STATUS]') || line.includes('[RESULT]')) lineStyle = "text-yellow-400";
                             else if (line.includes('[OPS]') || line.includes('[DATA_VIEW]') || line.includes('[SCAN]')) lineStyle = "text-blue-400";
