  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#050505] border border-cyan-500/50 p-2 rounded shadow-lg">
        <p className="font-mono text-cyan-400 text-xs tracking-wider uppercase">{payload[0].payload.subject}</p>
        <p className="font-mono text-gray-400 text-[10px] mt-1">CAPACITY: {payload[0].value}%</p>
      </div>
    );
  }
  return null;
...
const SecurityRadarChart = () => {
  ...
  return (
    <div ref={containerRef} className="w-full h-full min-h-[300px] flex flex-col items-center justify-center bg-transparent relative">
      <motion.div
      ...
      <div className="absolute top-4 right-4 text-[10px] text-gray-600 font-mono text-right">
        [DIAGNOSTIC: MULTI-AXIS ANALYSIS]<br/>
