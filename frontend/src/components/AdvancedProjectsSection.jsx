    ...
    >
      <div className="p-5">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <ChevronRight
            className={`h-4 w-4 text-gray-600 flex-shrink-0 mt-1 transition-transform duration-200 ${isExpanded ? "rotate-90" : "group-hover:translate-x-0.5"}`}
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-3 ml-7">
          <Badge
            className={`${style.badge} text-[10px] px-1.5 py-0 h-5 font-normal border`}
          >
            {project.category}
          </Badge>
          <Badge
            className={`${DIFFICULTY_STYLES[project.difficulty] || "bg-gray-500/10 text-gray-400 border-gray-500/30"} text-[10px] px-1.5 py-0 h-5 font-normal border`}
          >
            {project.difficulty}
          </Badge>
        ...
        <div className="flex flex-wrap gap-1 ml-7">
          {project.stack.map((tech, idx) => (
            <span
              key={idx}
              className="text-[10px] px-1.5 py-0.5 bg-[#111] border border-gray-800 rounded text-gray-500 font-mono"
            >
              {tech}
            </span>
          ))}
        ...
        {isExpanded && (
          <div className="mt-4 ml-7 space-y-4 border-t border-gray-800/50 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
              {project.highlights.map((h, idx) => (
                <div key={idx} className="flex items-start gap-2">
                  <CheckCircle2
                    className={`h-3 w-3 ${style.accent} mt-0.5 flex-shrink-0 opacity-70`}
                  />
            ...
            {/* Impact */}
            <div
              className={`text-xs ${style.accent} font-mono opacity-80 bg-[#080808] rounded px-3 py-2 border border-gray-800/50`}
            >
              {project.impact}
            </div>
          </div>
...
const AdvancedProjectsSection = ({ recruiterMode }) => {
  ...
  const expertCount = advancedProjects.filter(
    (p) => p.difficulty === "Expert",
  ).length;
  const enterpriseCount = advancedProjects.filter(
    (p) => p.type === "Enterprise",
  ).length;
  ...
  return (
    <section
      ...
      className="px-4 py-20 bg-[#050505]"
      data-testid="advanced-projects-section"
    >
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10">
          <h2 className="text-2xl font-bold mb-2 font-mono tracking-tight">
            ...
            <span className="text-white">Advanced Technical</span>{" "}
