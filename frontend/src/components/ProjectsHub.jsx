...
const ProjectsHub = ({ recruiterMode }) => {
  ...
  const getThumbnail = (project, style) => {
    ...
    return (
      <div className="w-full h-48 bg-[#050505] flex flex-col items-center justify-center border-b border-gray-800 relative overflow-hidden group-hover:scale-105 transition-transform duration-500">
        ...
        <IconComp
          className={`w-16 h-16 ${style.accent} opacity-20 mb-3 group-hover:opacity-40 transition-opacity duration-300`}
        />
        <span className="text-gray-600 font-mono text-xs opacity-50">
          &lt; /&gt;
        </span>
        <div
          className={`absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-${style.accent.split("-")[1]}-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
        ></div>
      </div>
    );
  };
  ...
  return (
    <section
      ...
      data-testid="projects-hub-section"
    >
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="mb-12">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500 font-mono mt-4">
            <span className="px-2 py-1 bg-[#0a0a0a] rounded border border-gray-800">
              {allProjects.length} Total Projects
            </span>
            <span className="hidden sm:inline w-px h-3 bg-gray-800" />
            <span className="hidden sm:inline">
              {projectCategories.length - 1} Specializations
            </span>
          </div>
        </div>
        ...
        {/* Category Tabs */}
        <div
          className="flex overflow-x-auto md:flex-wrap gap-2 mb-10 border-b border-gray-800/50 pb-3 hide-scrollbar whitespace-nowrap"
        ...
        >
          {projectCategories.map((cat) => (
            ...
            >
              {cat}
              <span
                className={`ml-2 text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === cat ? "bg-gray-800 text-white" : "bg-[#050505]"}`}
              >
                {tabCounts[cat]}
              </span>
              {activeTab === cat && (
          ...
          <div className="sm:hidden flex overflow-x-auto gap-4 pb-8 snap-x snap-mandatory hide-scrollbar -mx-4 px-4">
            {filteredProjects.map((project) => {
              ...
              return (
                ...
                >
                  <div
                    className={`holo-card bg-[#080808] border ${style.border?.replace("l-", "")} border-opacity-30 rounded-xl overflow-hidden`}
                  >
                    <ProjectCardContent
                      project={project}
                      style={style}
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                          ...
                          <div className="bg-[#0a0a0a] rounded-lg p-5 border border-gray-800/50 relative overflow-hidden">
                            <div
                              className={`absolute left-0 top-0 bottom-0 w-1 ${style.bg ? style.bg.replace("bg-", "") : "bg-gray-700"}`}
                            ></div>
                            <h4 className="text-[10px] uppercase font-mono tracking-widest text-gray-500 mb-2 flex items-center gap-2">
                              <span
                                className={`w-2 h-2 rounded-full ${style.accent.replace("text-", "bg-")} animate-pulse`}
                              ></span>
                              [S] Situation & Problem
                            </h4>
                            <p className="text-sm text-gray-300 leading-relaxed">
                              {selectedProject.problem ||
                                ...
                                "Identified security gaps and infrastructure vulnerabilities requiring immediate architectural redesign."}
                            </p>
                          </div>
                              ...
                              <ul className="space-y-3">
                                {(
                                  ...
                                  selectedProject.highlights || [
                                    ...
                                    "Deployed continuous monitoring.",
                                  ]
                                ).map((sol, idx) => (
                                  <li
                                    key={idx}
                        ...
                        <div className="space-y-6">
                          {/* Impact */}
                          <div
                            className={`bg-[#080808] rounded-lg p-5 border ${style.border} group`}
                          >
                            <h4
                              className={`text-[10px] uppercase font-mono tracking-widest ${style.accent} mb-3`}
                            >
                              [R] Result / Impact
                            </h4>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {selectedProject.stack.map((tech, i) => (
                                <span
                                  key={i}
                                  className="text-xs px-2.5 py-1 bg-gray-900 border border-gray-800 rounded-md text-gray-400 font-mono"
                                >
                                  {tech}
                                </span>
                              ))}
                          ...
                          <div className="pt-4 border-t border-gray-800/80 flex flex-col gap-3">
                            {selectedProject.demoUrl &&
                              selectedProject.demoUrl !== "#" && (
                                <button
                                  className={`w-full py-2.5 rounded-md font-mono text-xs font-bold text-black ${style.accent.replace("text-", "bg-")} hover:brightness-110 flex items-center justify-center gap-2 transition-all`}
                                >
                            ...
                            {(!selectedProject.demoUrl ||
                              ...
                              (!selectedProject.repoUrl ||
                                ...
                                <button
                                  disabled
                                  className="w-full py-2.5 rounded-md font-mono text-xs text-gray-600 border border-gray-800 cursor-not-allowed flex items-center justify-center gap-2 relative overflow-hidden"
                                >
...
const ProjectCardContent = ({ project, style, getThumbnail }) => (
  <>
    ...
    <div className="overflow-hidden relative">
      ...
      <div
        className={`absolute top-3 right-3 ${style.badge} px-2 py-1 rounded text-[10px] font-mono border backdrop-blur-md`}
      >
        {project.type}
      </div>
    </div>
    ...
    <div className="p-5 flex flex-col flex-grow">
      ...
      <p className="text-xs text-gray-500 leading-relaxed mb-4 line-clamp-2 flex-grow">
        {project.subtitle ||
          project.problem ||
          "Advanced implementation requiring complex architectural decisions."}
      </p>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {project.stack.slice(0, 4).map((tech, i) => (
          <span
            key={i}
            className="text-[10px] px-2 py-0.5 bg-[#111] border border-gray-800 rounded-full text-gray-400 font-mono"
          >
            {tech}
          </span>
        ))}
      ...
      <div className="flex justify-between items-center pt-3 border-t border-gray-900 mt-auto">
        <span
          className={`text-[10px] uppercase font-mono tracking-wider ${style.accent} flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity`}
        >
          <ChevronRight className="w-3 h-3" /> Execute Deep Dive
        </span>
        <Badge
          className={`${DIFFICULTY_STYLES[project.difficulty] || ""} text-[9px] px-1.5 py-0 h-4 border font-mono uppercase tracking-wider`}
        >
          {project.difficulty}
        </Badge>
      </div>
