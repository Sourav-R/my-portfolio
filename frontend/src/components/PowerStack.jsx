...
import { Terminal, Database, Server, Cloud, ShieldAlert, Key, ChevronRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import TiltCard from './TiltCard';
import { useTerminal } from '../context/TerminalContext';

const powerStack = [
  { id: 'linux', name: 'Linux Admin', icon: Terminal, role: 'OS / ENVIRONMENT', tab: 'Infrastructure & Network', log: '$ uname -a\nLinux thiru-soc 6.1.0 #1 SMP x86_64 GNU/Linux\n$ systemctl status sshd\n● active (running) since Mon; 99.9% uptime' },
  { id: 'elk', name: 'ELK Stack', icon: Database, role: 'SIEM / ANALYTICS', tab: 'Infrastructure & Network', log: '$ curl -s localhost:9200/_cluster/health | jq .status\n"green"\n$ elasticsearch-service status\nCluster: thiru-prod | Nodes: 3 | Shards: 42' },
  { id: 'python', name: 'Python', icon: Server, role: 'AUTOMATION / SCRIPTS', tab: 'Software Development', log: '$ python3 --version\nPython 3.11.6\n$ wc -l ~/scripts/**/*.py\n12,847 total lines across 89 automation scripts' },
  { id: 'aws', name: 'AWS Cloud', icon: Cloud, role: 'INFRASTRUCTURE', tab: 'Cloud Engineering', log: '$ aws sts get-caller-identity\nAccount: ****-thiru-prod\n$ aws ec2 describe-instances --query "Reservations[].Instances[].State"\nrunning (7 instances)' },
  { id: 'burp', name: 'Burp Suite', icon: ShieldAlert, role: 'VAPT / PROXY', tab: 'Offensive Security', log: '$ burpsuite --project=jago-vapt\n[SCANNER] Active scan: 142 requests queued\n[FINDINGS] 3 High, 7 Medium, 12 Info\n[PROXY] Intercepting on 127.0.0.1:8080' },
  { id: 'tpot', name: 'T-Pot Deception', icon: Key, role: 'ACTIVE DEFENSE', tab: 'Infrastructure & Network', log: '$ docker ps | grep honeypot\n30+ active decoy containers\n$ tail -f fakebank/auth.log\n[ALERT] Malicious IP logged' },
];
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="mb-8">
          <h2 className="text-2xl md:text-3xl font-bold mb-2">
            <span className="text-white">Power</span> <span className="text-cyan-500">Stack</span>
          </h2>
          <p className="text-gray-500 font-mono text-sm max-w-md">&gt; Top 6 Daily Drivers for enterprise engineering and offensive security operations.</p>
        </div>
              ...
              >
                <TiltCard intensity={10} scale={1.03} className="h-full">
                  <div className="glass-card p-5 flex flex-col items-center justify-center text-center group h-full min-h-[120px]">
                    ...
                    <h3 className="text-gray-100 font-bold text-sm mb-1 group-hover:text-cyan-300 transition-colors">{tool.name}</h3>
                    <p className="text-[9px] font-mono text-gray-600 uppercase tracking-widest">{tool.role}</p>
                  </div>
                </TiltCard>
              </motion.div>
            );
