  ...
  useFrame(() => {
    if (groupRef.current) {
      ...
      if (!isDragging.current) {
        ...
        if (Math.abs(dragVelocity.current.x) < 0.001)
          dragVelocity.current.x = 0.002;
      }
    }
  });
          ...
          <div className="w-full h-full flex flex-col items-center justify-center p-6 bg-[#0a0a0a] rounded-xl border border-rose-500/20 shadow-[inset_0_0_50px_rgba(225,29,72,0.05)]">
            <div className="flex items-center gap-2 mb-6">
              ...
              <div className="text-rose-400 font-mono text-xs uppercase tracking-widest font-bold">
                Secure Node Recovery - 2D Fallback Active
              </div>
            </div>
            <div className="flex flex-wrap gap-2 justify-center max-h-64 overflow-y-auto w-full custom-scrollbar pr-2">
              {tools.map((tool, i) => (
                <span
                  key={i}
                  className="px-3 py-1 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-xs font-mono rounded hover:bg-cyan-500/20 hover:border-cyan-500/50 hover:shadow-[0_0_10px_rgba(6,182,212,0.3)] hover:text-white transition-all cursor-default"
                >
                  {tool}
                </span>
              ))}
          ...
          <Canvas3DErrorBoundary
            fallback={
              <div className="w-full h-full flex items-center justify-center text-rose-500 font-mono text-xs text-center px-4 border border-rose-500/20 rounded-xl bg-[#0a0a0a]">
                3D render initialization failed.
                <br /> Falling back to structural memory.
              </div>
            }
          >
